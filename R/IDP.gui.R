...
IDP.gui<-function(idp) { 
  ...
  if (idp$gui$modal) {
    ...
    idp$gui$win<-gbasicdialog(title=paste("Isodate File Processor -", IDP.getVersionInfo(idp)), do.buttons=FALSE, visible=FALSE, parent=idp$gui$parent)
    size(idp$gui$win) <- c(1280, 750)
  } else {
    cat("\nStarting IDP in window mode.\n")
    idp$gui$win<-gwindow(paste("Isodate File Processor -", IDP.getVersionInfo(idp)), width=1280, height=750, visible=FALSE, parent=idp$gui$parent)
  }
